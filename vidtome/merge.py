...
def visualize_token_merging(src_tokens, dst_tokens, merged_tokens, ratio, mode="mean"):
    """
    Visualizes the merging process of source and destination tokens.
    
    Parameters:
    - src_tokens (np.array): Source tokens as a 2D array of shape (N, C), where N is the number of tokens and C is the number of features.
    - dst_tokens (np.array): Destination tokens as a 2D array of shape (M, C), where M is the number of tokens and C is the number of features.
    - merged_tokens (np.array): Tokens after merging, with the same shape as dst_tokens.
    - ratio (float): Ratio of tokens that were merged from source into destination.
    - mode (str): The mode of merging, "mean" or "replace".
    """
    # Create a figure with three subplots: original src, original dst, and merged result
    fig, axes = plt.subplots(1, 3, figsize=(15, 5))
    fig.suptitle(f"Token Merging - Mode: {mode} | Ratio: {ratio*100}%", fontsize=16)

    # Visualize Source Tokens
    axes[0].scatter(np.arange(src_tokens.shape[0]), np.zeros_like(np.arange(src_tokens.shape[0])), c=src_tokens[:, 0], cmap='viridis')
    axes[0].set_title("Source Tokens")
    axes[0].set_xlabel("Token Index")
    axes[0].set_ylabel("Token Feature Value")
    
    # Visualize Destination Tokens
    axes[1].scatter(np.arange(dst_tokens.shape[0]), np.zeros_like(np.arange(dst_tokens.shape[0])), c=dst_tokens[:, 0], cmap='viridis')
    axes[1].set_title("Destination Tokens")
    axes[1].set_xlabel("Token Index")
    axes[1].set_ylabel("Token Feature Value")
    
    # Visualize Merged Tokens
    axes[2].scatter(np.arange(merged_tokens.shape[0]), np.zeros_like(np.arange(merged_tokens.shape[0])), c=merged_tokens[:, 0], cmap='viridis')
    axes[2].set_title("Merged Tokens")
    axes[2].set_xlabel("Token Index")
    axes[2].set_ylabel("Token Feature Value")
...
# np.random.seed(42)
# src_tokens = np.random.rand(10, 5)  # 10 source tokens with 5 features each
# dst_tokens = np.random.rand(15, 5)  # 15 destination tokens with 5 features each

# # For merging, we simulate a "mean" merge
# merged_tokens = (src_tokens[:5] + dst_tokens[:5]) / 2  # Assume the first 5 src tokens are merged with the first 5 dst tokens

# # Visualize the merging process with a ratio of 50% merged
# visualize_token_merging(src_tokens, dst_tokens, merged_tokens, ratio=0.5, mode="mean")
